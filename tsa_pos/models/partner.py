...
class Partner(StandardModel,Base):
    ...
    provinsi_id = Column(ForeignKey('provinsi.id', ondelete='RESTRICT'), nullable=False)
    kota_id = Column(ForeignKey('kota.id', ondelete='RESTRICT'), nullable=False)
    kecamatan_id = Column(ForeignKey('kecamatan.id', ondelete='RESTRICT'), nullable=False)
    ...
    # invoice = relationship('Invoices', back_populates='partner', passive_deletes=True)
