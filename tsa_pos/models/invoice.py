...
class Invoices(StandardModel, Base):
    ...
    category_id = Column(
        ForeignKey("invoice_category.id", ondelete="RESTRICT"), nullable=False
    )
    category = relationship("InvoiceCategory", backref=backref("invoices"))
    name = Column(String(128))
    code = Column(String(128))
    amount = Column(Float)
    est_delivery = Column(DateTime)
    invoice_date = Column(DateTime)
    partner_id = Column(ForeignKey("partner.id", ondelete="RESTRICT"), nullable=False)
...
class InvoiceItems(Base):
    ...
    order_item_id = Column(
        ForeignKey("order_items.id", ondelete="CASCADE"), nullable=True
    )
    order_id = Column(ForeignKey("order_items.id", ondelete="CASCADE"), nullable=True)
    product = relationship(
        "Product", back_populates="invoice_items", passive_deletes=True
    )
    invoice = relationship(
        "Invoices", back_populates="invoice_items", passive_deletes=True
    )
    __table_args__ = (
        PrimaryKeyConstraint("invoice_id", "product_id", name="invoice_items_pkey"),
    )
